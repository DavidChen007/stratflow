...
const App: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsSaving(false);
      alert('持久化保存失败，请检查后端 API 服务。');
    }
  };

  const deleteProcess = async (id: string) => {
    if (!authenticatedEnt) return;
    if (confirm('确定要从数据库中永久移除该流程资产吗？')) {
      try {
      ...
      <div className="min-h-screen w-full flex items-center justify-center bg-slate-900 p-6">
        <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-10">
          ...
          <div className="flex bg-slate-100 p-1 rounded-2xl mb-6">
            <button onClick={() => setIsRegistering(false)} className={`flex-1 py-2 text-xs font-black uppercase rounded-xl transition-all ${!isRegistering ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-400'}`}>登录</button>
          ...
          <div className="space-y-4">
            ...
            {isRegistering ? (
              ...
              <>
                <input className="w-full p-4 bg-slate-50 border rounded-2xl text-sm font-bold outline-none focus:border-brand-500 transition-colors" placeholder="账号 (admin)" value={loginUsername} onChange={e => setLoginUsername(e.target.value)} />
                <input type="password"  className="w-full p-4 bg-slate-50 border rounded-2xl text-sm font-bold outline-none focus:border-brand-500 transition-colors" placeholder="密码 (888888)" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} />
              </>
            )}
        ...
        {showRegSuccess && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-6">
            <div className="bg-white rounded-[2.5rem] p-10 max-w-md w-full text-center shadow-2xl">
              ...
              <button onClick={() => setShowRegSuccess(null)} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase">返回并登录</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="flex h-screen w-screen bg-slate-50 overflow-hidden">
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col shrink-0 p-4">
        ...
        <nav className="flex-1 space-y-1">
          ...
          <SidebarBtn active={activeTab === 'user'} onClick={() => setActiveTab('user')} icon={<UserCog />} label="治理权限中心" />
        </nav>
        ...
        <header className="h-16 bg-white border-b flex items-center justify-between px-8 shrink-0 shadow-sm z-10">
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleSave} disabled={isSaving} className={`px-6 py-2 rounded-xl text-xs font-black uppercase transition-all shadow-lg flex items-center gap-2 ${showSaveSuccess ? 'bg-emerald-500 text-white scale-105' : 'bg-brand-600 text-white hover:bg-brand-700'}`}>
              ...
              {showSaveSuccess ? '同步成功' : (isSaving ? '正在保存...' : '持久化存档')}
            </button>
          </div>
        </header>
