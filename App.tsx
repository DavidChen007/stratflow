...
import { AppState, ProcessDefinition, Enterprise, User } from './types';
      ...
      <div className="min-h-screen w-full flex items-center justify-center bg-slate-900 p-6">
        <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-10 animate-in zoom-in-95 duration-300">
          <div className="text-center mb-8">
            ...
            <p className="text-[10px] font-black text-slate-400 tracking-widest uppercase mt-1">Digital Asset Governance</p>
          </div>
          <div className="flex bg-slate-100 p-1 rounded-2xl mb-6">
            <button onClick={() => setIsRegistering(false)} className={`flex-1 py-2 text-xs font-black uppercase rounded-xl transition-all ${!isRegistering ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-400'}`}>身份登录</button>
            <button onClick={() => setIsRegistering(true)} className={`flex-1 py-2 text-xs font-black uppercase rounded-xl transition-all ${isRegistering ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-400'}`}>空间入驻</button>
          </div>
          <div className="space-y-4">
            <input className="w-full p-4 bg-slate-50 border rounded-2xl text-sm font-bold outline-none focus:border-brand-500" placeholder="企业 ID (如: demo-corp)" value={entId} onChange={e => setEntId(e.target.value)} />
            {isRegistering ? (
              ...
              <>
                <input className="w-full p-4 bg-slate-50 border rounded-2xl text-sm font-bold outline-none focus:border-brand-500" placeholder="账号" value={loginUsername} onChange={e => setLoginUsername(e.target.value)} />
                <input type="password"  className="w-full p-4 bg-slate-50 border rounded-2xl text-sm font-bold outline-none focus:border-brand-500" placeholder="密码" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} />
              </>
            )}
            <button onClick={isRegistering ? async () => { await saveEnterprise({ name: entId, displayName: entDisplayName }); setIsRegistering(false); alert("入驻成功！默认管理员: admin / 密码: 888888"); } : handleLogin} className="w-full bg-brand-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:bg-brand-700 transition-all">
              {isRegistering ? '开启治理空间' : '建立安全会话'}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="flex h-screen w-screen bg-slate-50 overflow-hidden">
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col shrink-0 p-4">
        ...
        <nav className="flex-1 space-y-1">
          ...
          <SidebarBtn active={activeTab === 'okr'} onClick={() => setActiveTab('okr')} icon={<Target />} label="战略 OKR" />
          <SidebarBtn active={activeTab === 'weekly'} onClick={() => setActiveTab('weekly')} icon={<Calendar />} label="PAD 周治理" />
        ...
        <button onClick={() => { setAuthenticatedEnt(null); setCurrentUser(null); }} className="p-4 text-xs text-red-400 font-bold flex items-center gap-2 hover:bg-slate-800 rounded-xl transition-all"><LogOut size={16}/>注销登录</button>
      </aside>
        ...
        <header className="h-16 bg-white border-b flex items-center justify-between px-8 shrink-0 shadow-sm z-10">
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleSave} disabled={isSaving} className={`px-6 py-2 rounded-xl text-xs font-black uppercase transition-all shadow-lg flex items-center gap-2 ${showSaveSuccess ? 'bg-emerald-500 text-white scale-105' : 'bg-brand-600 text-white hover:bg-brand-700'}`}>
              ...
              {showSaveSuccess ? '已存档' : (isSaving ? '正在同步' : '云端同步')}
            </button>
          </div>
        </header>
